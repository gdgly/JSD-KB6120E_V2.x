...
{
	bool state; 
	state = SDEload("\\SD_TSP\\TSP.TXT", ( FilePageSize_TSP * FileNum ), pFile, sizeof(  struct uFile_TSP ));
...
void	File_Load_R24( uint16_t FileNum, struct uFile_R24 * pFile )
{
	...
	state = SDEload("\\SD_R24\\R24.TXT", ( FilePageSize_R24 * FileNum ), pFile, sizeof( struct uFile_R24 ));
	if ( ! state )
	{
		pFile->sample_begin = 0;
	}	
...
void	File_Load_SHI( uint16_t FileNum, struct uFile_SHI * pFile )
{
	...
	bool state;   
	state = SDEload("\\SD_SHI\\SHI.TXT",  ( FilePageSize_SHI * FileNum ), pFile, sizeof( struct uFile_SHI ));
...
void	File_Save_TSP( uint16_t FileNum, struct uFile_TSP const * pFile )
{
	...
	SDEsave("\\SD_TSP\\TSP.TXT", ( FilePageSize_TSP * FileNum ), ss, FilePageSize_TSP );
}
...
void	File_Save_R24( uint16_t FileNum, struct uFile_R24 const * pFile )
{
	...
	memcpy( ss, pFile, sizeof( *pFile ));
	assert(( FileNum >= 1 ) && ( FileNum <= FileNum_Max ));
	SDEsave("\\SD_R24\\R24.TXT", ( FilePageSize_R24 * FileNum ),ss, FilePageSize_R24 );
}
...
void	File_Save_SHI( uint16_t FileNum, struct uFile_SHI const * pFile )
{
	...
	memcpy( ss, pFile, sizeof( *pFile ));
	assert(( FileNum >= 1 ) && ( FileNum <= FileNum_Max ));
	SDEsave("\\SD_SHI\\SHI.TXT",  ( FilePageSize_SHI  * FileNum ), ss, FilePageSize_SHI );
}
/**/
...
void	PowerLogSave_PowerBoot( void )
{
	...
	SDEsave("\\SD_POW\\POWER.TXT", x_PowerLogBase + ( PowerLogSize * PowerLogIndex ) + 4 , powertime, 4);
	...
	SDEsave("\\SD_POW\\POWER.TXT", x_PowerLogBase + ( PowerLogSize * PowerLogIndex ) , powertime, 4);
}
...
{	
	...
	if( PowerLogIndex > index )
		...
		i = 0;
		SDEload("\\SD_POW\\POWER.TXT",x_PowerLogBase + ( PowerLogSize * i ),  pLog, 8 );
}
